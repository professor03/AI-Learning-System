...
![教材摘要與章節詳解的既有展示畫面](docs/assets/01-live-notes.png)

[**初學者圖文指南：安裝、讀筆記、作答、複習與 LearnSight**](docs/visual-guide.md)。每張圖都附上操作順序、預期結果與功能限制；截圖為先前保存的展示，不代表本次重新測試 AI 生成。

## 專案亮點

- **AI 學習流程**：從教材或筆記建立學習內容，產生摘要、問答與測驗。
- **間隔複習**：以 FSRS-style 排程記錄答題結果，安排下一次複習。
- **LearnSight 整合**：連接 YOLO 人物偵測服務，支援開始、同步與結束學習工作階段。
...
有金鑰與 YOLO 服務後，可依下列流程展示（影像來源也可用本機影片）：
...
本專案不是只展示畫面，而是把 AI 教學、學習科學（間隔複習）與人物偵測訊號串成一個可操作的研究原型。後續可朝學習成效評估、個人化推薦與多模態學習分析延伸。

## 快速導覽與展示圖

本系統以教材或筆記為輸入，依序建立摘要、完成測驗、安排後續複習，並可選擇將 LearnSight 的聚合人物訊號同步到學習時段。這是可操作的整合流程，不把人物數量解釋為專注度、理解程度或學習成效。

| 想看什麼 | 操作入口 |
| --- | --- |
| 不需金鑰的快速體驗 | 首頁選「載入離線示範教材」 |
| 筆記與摘要 | 筆記 → 選擇教材 → 閱讀或產生摘要 |
| 答題回饋 | 測驗 → 完成示範題或教材題目 |
| 複習規劃 | 間隔複習 → 完成卡片並查看下一次複習日 |
| 視覺服務整合 | LearnSight → 開始、同步、結束工作階段 |

### 首次展示的六步流程

1. 執行 npm ci，再執行 npm run dev。
2. 開啟終端輸出的本機網址；首頁載入離線示範教材。
3. 在筆記頁閱讀摘要。
4. 在測驗頁完成題目，查看回饋。
5. 在間隔複習頁完成卡片，確認下一次複習時間更新。
6. YOLO 服務已啟動時，到 LearnSight 建立時段、同步人數訊號並結束時段。

離線示範內容為預先編寫，用來驗證介面與流程，不能當作 Gemini 生成品質的實驗結論。

### 本機展示截圖

| AI 筆記摘要 | 答題回饋 | LearnSight 連線 |
| --- | --- | --- |
| ![AI 筆記摘要](docs/assets/01-live-notes.png) | ![測驗回饋](docs/assets/02-quiz-feedback.png) | ![LearnSight](docs/assets/03-live-learnsight.png) |
